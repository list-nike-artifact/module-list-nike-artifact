...
from parameters import primes_per_order,good_primes,bad_primes
...
lgp = []
lbp = []
for p_current in Primes():
    if(p_current%4==1):
        lgp.append(p_current)
    else:
        lbp.append(p_current)
    if(len(lgp)>=good_primes) and (len(lbp)>=1+bad_primes):
        break
lbp = lbp[1:bad_primes+1]
lgp = lgp[0:good_primes]
...
objs = ObjectFormatter([
        ...
        Object('uint32_t[]', 'QUAT_good_primes_list', [int(v) for v in lgp]),
        Object('uint32_t[]', 'QUAT_bad_primes_list', [int(v) for v in lbp]),
