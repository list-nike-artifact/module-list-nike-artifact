...
static inline void
select_base_change_matrix(basis_change_matrix_fp2_t *M,
                          const basis_change_matrix_fp2_t *M1,
...
static inline void
choose_index_theta_point_fp2_t(fp2_t *res, int ind, const theta_point_fp2_t *T)
{
...
static void
apply_isomorphism_general(theta_point_fp2_t *res,
                          const basis_change_matrix_fp2_t *M,
                          const theta_point_fp2_t *P,
                          const bool Pt_not_zero)
{
    fp2_t x1;
    theta_point_fp2_t temp;
...
static void
apply_isomorphism(theta_point_fp2_t *res, const basis_change_matrix_fp2_t *M, const theta_point_fp2_t *P)
{
...
static void
base_change_matrix_multiplication(basis_change_matrix_fp2_t *res,
                                  const basis_change_matrix_fp2_t *M1,
                                  const basis_change_matrix_fp2_t *M2)
{
    basis_change_matrix_fp2_t tmp;
    fp2_t sum, m_ik, m_kj;
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            fp2_set_zero(&sum);
...
// compute the theta_point_fp2_t corresponding to the couple of point T on an elliptic product
static void
base_change(theta_point_fp2_t *out, const theta_gluing_fp2_t *phi, const theta_couple_point_fp2_t *T)
{
...
static void
action_by_translation_z_and_det(fp2_t *z_inv, fp2_t *det_inv, const xpoint_eqw_fp2_t *P4, const xpoint_eqw_fp2_t *P2)
{
...
static void
action_by_translation_compute_matrix(translation_matrix_fp2_t *G,
                                     const xpoint_eqw_fp2_t *P4,
                                     const xpoint_eqw_fp2_t *P2,
                                     const fp2_t *z_inv,
                                     const fp2_t *det_inv)
{
    fp2_t tmp;
...
static int
verify_two_torsion(const theta_couple_point_fp2_t *K1_2, const theta_couple_point_fp2_t *K2_2, const theta_couple_curve_fp2_t *E12)
{
    // First check if any point in K1_2 or K2_2 is zero, if they are then the points did not have
    // order 8 when we started gluing
    if (xpoint_eqw_fp2_is_zero(&K1_2->P1) | xpoint_eqw_fp2_is_zero(&K1_2->P2) | xpoint_eqw_fp2_is_zero(&K2_2->P1) | xpoint_eqw_fp2_is_zero(&K2_2->P2)) {
        return 0;
    }

    // Now ensure that P1, Q1 and P2, Q2 are independent. For points of order two this means
    // that they're not the same
    if (xpoint_eqw_fp2_is_equal(&K1_2->P1, &K2_2->P1) | xpoint_eqw_fp2_is_equal(&K1_2->P2, &K2_2->P2)) {
        return 0;
    }

    // Finally, double points to ensure all points have order exactly 0
    theta_couple_point_fp2_t O1, O2;
    double_couple_point_fp2(&O1, K1_2, E12);
    double_couple_point_fp2(&O2, K2_2, E12);
    // If this check fails then the points had order 2*f for some f, and the kernel is malformed.
    if (!(xpoint_eqw_fp2_is_zero(&O1.P1) & xpoint_eqw_fp2_is_zero(&O1.P2) & xpoint_eqw_fp2_is_zero(&O2.P1) & xpoint_eqw_fp2_is_zero(&O2.P2)))
        return 0;
...
// and 1 otherwise
static int
action_by_translation(translation_matrix_fp2_t *Gi,
                      const theta_couple_point_fp2_t *K1_4,
                      const theta_couple_point_fp2_t *K2_4,
                      const theta_couple_curve_fp2_t *E12)
{
    // Compute points of order 2 from Ki_4
    theta_couple_point_fp2_t K1_2, K2_2;
    double_couple_point_fp2(&K1_2, K1_4, E12);
    double_couple_point_fp2(&K2_2, K2_4, E12);
...
static int
gluing_change_of_basis(basis_change_matrix_fp2_t *M,
                       const theta_couple_point_fp2_t *K1_4,
                       const theta_couple_point_fp2_t *K2_4,
                       const theta_couple_curve_fp2_t *E12)
{
 ...
 *
 * @param out Output: the theta_gluing_fp2_t
 ...
 */
static int
gluing_compute(theta_gluing_fp2_t *out,
               const theta_couple_curve_fp2_t *E12,
               const theta_couple_jac_point_fp2_t *xyK1_8,
               const theta_couple_jac_point_fp2_t *xyK2_8,
               bool verify)
{
    // Ensure that we have been given the eight torsion
#ifndef NDEBUG
    {
        int check = point_jac_fp2_test_order_twof(&xyK1_8->P1, &E12->E1, 3);
        if (!check)
            debug_print("xyK1_8->P1 does not have order 8");
        check = point_jac_fp2_test_order_twof(&xyK2_8->P1, &E12->E1, 3);
        if (!check)
            debug_print("xyK2_8->P1 does not have order 8");
        check = point_jac_fp2_test_order_twof(&xyK1_8->P2, &E12->E2, 3);
        if (!check)
            debug_print("xyK2_8->P1 does not have order 8");
        check = point_jac_fp2_test_order_twof(&xyK2_8->P2, &E12->E2, 3);
        if (!check)
    ...
    theta_couple_jac_point_fp2_t xyK1_4, xyK2_4;

    double_couple_jac_point_fp2(&xyK1_4, xyK1_8, E12);
    double_couple_jac_point_fp2(&xyK2_4, xyK2_8, E12);

    // Convert from (X:Y:Z) coordinates to (X:Z)
    theta_couple_point_fp2_t K1_8, K2_8;
    theta_couple_point_fp2_t K1_4, K2_4;

    couple_point_jac_to_ec_fp2(&K1_8, xyK1_8);
    couple_point_jac_to_ec_fp2(&K2_8, xyK2_8);
    couple_point_jac_to_ec_fp2(&K1_4, &xyK1_4);
    couple_point_jac_to_ec_fp2(&K2_4, &xyK2_4);
    ...
    theta_point_fp2_t TT1, TT2;

    base_change(&TT1, out, &K1_8);
    base_change(&TT2, out, &K2_8);

    // compute the codomain
    to_squared_theta_fp2(&TT1, &TT1);
    to_squared_theta_fp2(&TT2, &TT2);
    ...
    hadamard_fp2(&out->codomain, &out->codomain);
    return 1;
}

// sub routine of the gluing eval
static void
gluing_eval_point(theta_point_fp2_t *image, const theta_couple_jac_point_fp2_t *P, const theta_gluing_fp2_t *phi)
{
    // #ifndef NDEBUG
    //     uint32_t fp2_mul_start = N_FP2_MUL;
    //     uint32_t fp2_sqr_start = N_FP2_SQR;
    // #endif

    theta_point_fp2_t T1, T2;
    add_components_fp2_t add_comp1, add_comp2;

    // Compute the cross addition components of P1+Q1 and P2+Q2
    point_jac_fp2_to_xz_add_components(&add_comp1, &P->P1, &phi->xyK1_8.P1, &phi->domain.E1);
    point_jac_fp2_to_xz_add_components(&add_comp2, &P->P2, &phi->xyK1_8.P2, &phi->domain.E2);
    ...
    hadamard_fp2(image, image);
}

/**
 * @brief Evaluate a gluing isogeny from an elliptic product on a basis
 *
 * @param image1 Output: the theta_point_fp2_t of the image of the first couple of points
 ...
 **/
static void
gluing_eval_basis(theta_point_fp2_t *image1,
                  theta_point_fp2_t *image2,
                  const theta_couple_jac_point_fp2_t *xyT1,
                  const theta_couple_jac_point_fp2_t *xyT2,
                  const theta_gluing_fp2_t *phi)
{
 ...
 *
 * @param out Output: the theta_isogeny_fp2
 ...
 */
static int
theta_isogeny_fp2_compute(theta_isogeny_fp2_t *out,
                      const theta_structure_fp2_t *A,
                      const theta_point_fp2_t *T1_8,
                      const theta_point_fp2_t *T2_8,
                      ...
                      bool verify)
{
    ...
    if (hadamard_bool_1) {
        hadamard_fp2(&TT1, T1_8);
        to_squared_theta_fp2(&TT1, &TT1);
        hadamard_fp2(&TT2, T2_8);
        to_squared_theta_fp2(&TT2, &TT2);
    } else {
        to_squared_theta_fp2(&TT1, T1_8);
        to_squared_theta_fp2(&TT2, T2_8);
    }
    ...
    if (fp2_is_zero(&TT2.x)
        ...
        debug_print(":: theta_isogeny_fp2_compute: Early splitting");
        return 0;
    }
    ...
    if (verify) {
        ...
        if (!fp2_is_equal(&t1, &t2)) {
            debug_print(":: theta_isogeny_fp2_compute: Precompute 1 failed");
            fp2_print("t1 ", &t1);
            fp2_print("t2 ", &t2);
            return 0;
        }
        ...
        if (!fp2_is_equal(&t1, &t2)) {
            debug_print(":: theta_isogeny_fp2_compute: Precompute 2 failed");
            fp2_print("t1 ", &t1);
            fp2_print("t2 ", &t2);
            return 0;
        }
        ...
        fp2_mul(&t2, &TT2.z, &out->precomputation.z);
        if (!fp2_is_equal(&t1, &t2)) {
            debug_print(":: theta_isogeny_fp2_compute: Precompute 3 failed");
            fp2_print("t1 ", &t1);
            fp2_print("t2 ", &t2);
            return 0;
        }
        ...
        if (!fp2_is_equal(&t1, &t2)) {
            debug_print(":: theta_isogeny_fp2_compute: Precompute 4 failed");
            fp2_print("t1 ", &t1);
            fp2_print("t2 ", &t2);
            return 0;
        }
    ...
    if (hadamard_bool_2) {
        hadamard_fp2(&out->codomain.null_point, &out->codomain.null_point);
    }
    return 1;
}

static void
theta_isogeny_fp2_eval(theta_point_fp2_t *out, const theta_isogeny_fp2_t *phi, const theta_point_fp2_t *P)
{
    if (phi->hadamard_bool_1) {
        hadamard_fp2(out, P);
        to_squared_theta_fp2(out, out);
    } else {
        to_squared_theta_fp2(out, P);
    }
...
static bool
splitting_compute(theta_splitting_fp2_t *out, const theta_structure_fp2_t *A, int zero_index)

{
    ...
    memset(&out->M, 0, sizeof(basis_change_matrix_fp2_t));
    ...
    for (int i = 0; i < 10; i++) {
        ...
        for (int t = 0; t < 4; t++) {
            ...
            choose_index_theta_point_fp2_t(&t2, t, &A->null_point);
            choose_index_theta_point_fp2_t(&t1, t ^ EVEN_INDEX[i][1], &A->null_point);
...
static int
theta_product_structure_to_elliptic_product(theta_couple_curve_fp2_t *E12, theta_structure_fp2_t *A)
{
    fp2_t xx, yy;

    // This should be true from our computations in splitting_compute
    // but still check this for sanity
    if (!is_product_theta_point_fp2_t(&A->null_point))
        return 0;

    curve_mg_fp2_init(&(E12->E1));
    curve_mg_fp2_init(&(E12->E2));
...
static int
theta_point_fp2_to_montgomery_point(theta_couple_point_fp2_t *P12, const theta_point_fp2_t *P, const theta_structure_fp2_t *A)
{
    fp2_t temp;
    const fp2_t *x, *z;

    if (!is_product_theta_point_fp2_t(P))
        return 0;
...
uint16_t
modified_balanced_strategy(uint16_t *strategy, uint32_t chain_length)
{
    // Caller's responsibility to allocate enough memory (MAX_STRATEGY_LENGTH)
    memset(strategy, 0, MAX_STRATEGY_LENGTH * sizeof(uint16_t));

    strategy[0] = chain_length;
    uint16_t current = 1;
    while (current < MAX_STRATEGY_LENGTH && strategy[current - 1] != 1) {

        // Even though the loop condition permits it, this assert is for logic
        // errors in how the strategy is computed: the algorithm should never
        // reach the end of the array
        assert(current < MAX_STRATEGY_LENGTH - 1);
        ...
        const unsigned num_dbls = strategy[current - 1] >= 16 ? strategy[current - 1] / 2 : strategy[current - 1] - 1;
...
static
uint32_t
_theta_chain_compute_impl_fp2(
    theta_couple_curve_fp2_t *E34,
    theta_couple_curve_fp2_t *E12,
    ...
    // the gluing step
    theta_gluing_fp2_t first_step;
    {
        assert(strategy[current] == 1);

        // compute the gluing isogeny
        if (!gluing_compute(&first_step, E12, &C1_jac_dbls[current], &C2_jac_dbls[current], verify)) {
        ...
        for (int j = 0; j < current; ++j) {
            gluing_eval_basis(&thetaQ1[j], &thetaQ2[j], &C1_jac_dbls[j], &C2_jac_dbls[j], &first_step);
            --strategy[j];
        }

        --current;
    }
        // printf(":: [2d isogeny] (Fp2) Gluing took %.3Lf ms\n", (long double)(clock() - gluing_start) * 1000 / CLOCKS_PER_SEC);
        // clock_t start_general_steps = clock();

    theta_point_fp_t thetaQ1_fp[MAX_STRATEGY_LENGTH], thetaQ2_fp[MAX_STRATEGY_LENGTH];

    for (uint32_t i = 0; i < strategy_length; i++) {
        theta_point_fp2_to_fp(&thetaQ1_fp[i], &thetaQ1[i]);
        theta_point_fp2_to_fp(&thetaQ2_fp[i], &thetaQ2[i]);
    }

    theta_structure_fp_t theta_fp;
    theta_point_fp2_to_fp(&theta_fp.null_point, &first_step.codomain);
    theta_fp.precomputation = 0;
    theta_precomputation_fp(&theta_fp);
    ...
    for (unsigned i = 1; current >= 0 && strategy[current]; ++i) {
        ...
        while (strategy[current] != 1) {
            ...
            assert(num_dbls && num_dbls < strategy[current - 1]);
            double_iter_fp(&thetaQ1_fp[current], &theta_fp, &thetaQ1_fp[current - 1], num_dbls);
            double_iter_fp(&thetaQ2_fp[current], &theta_fp, &thetaQ2_fp[current - 1], num_dbls);
            strategy[current] = strategy[current - 1] - num_dbls;
        }
        ...
        int ret;
        if (i == chain_length - 2)
            ret = theta_isogeny_fp_compute(&step, &theta_fp, &thetaQ1_fp[current], &thetaQ2_fp[current], 0, 0, verify);
        else if (i == chain_length - 1)
            ret = theta_isogeny_fp_compute(&step, &theta_fp, &thetaQ1_fp[current], &thetaQ2_fp[current], 1, 0, false);
        else
            ret = theta_isogeny_fp_compute(&step, &theta_fp, &thetaQ1_fp[current], &thetaQ2_fp[current], 0, 1, verify);
        ...
        theta_fp = step.codomain;

        // pushing the kernel
        assert(strategy[current] == 1);
        for (int j = 0; j < current; ++j) {
            theta_isogeny_fp_eval(&thetaQ1_fp[j], &step, &thetaQ1_fp[j]);
            theta_isogeny_fp_eval(&thetaQ2_fp[j], &step, &thetaQ2_fp[j]);
            assert(strategy[j]);
            --strategy[j];
        }
    ...
    assert(current == -1);
        ...
        // printf(":: [2d isogeny] (Fp) General steps %.3Lf ms\n", (long double)(clock() - start_general_steps) * 1000 / CLOCKS_PER_SEC);
        // clock_t splitting_start = clock();
    theta_structure_fp2_t theta;
    theta_structure_fp_to_fp2(&theta, &theta_fp);
    theta_point_fp_to_fp2(&theta.null_point, &theta_fp.null_point);

    // final splitting step
    theta_splitting_fp2_t last_step;
    ...
    if (!theta_product_structure_to_elliptic_product(E34, &last_step.B))
    {
        ...
        // printf(":: [2d isogeny] (Fp2) Splitting took %.3Lf ms\n", (long double)(clock() - splitting_start) * 1000 / CLOCKS_PER_SEC);
...
theta_chain_compute_fp2(
    ...
    theta_couple_curve_fp2_t E12_fp2, E34_fp2;
    curve_mg_fp2_init(&E12_fp2.E1);
    curve_mg_fp2_init(&E12_fp2.E2);
    curve_mg_fp2_init(&E34_fp2.E1);
    curve_mg_fp2_init(&E34_fp2.E2);

    curve_mg_fp_to_fp2(&E12_fp2.E1, &E12->E1);
    curve_mg_fp_to_fp2(&E12_fp2.E2, &E12->E2);
    curve_mg_fp_to_fp2(&E34_fp2.E1, &E34->E1);
    curve_mg_fp_to_fp2(&E34_fp2.E2, &E34->E2);

    uint32_t r = _theta_chain_compute_impl_fp2(
        &E34_fp2,
        &E12_fp2,
        chain_length,
        C1_jac_dbls,
    ...
    curve_mg_fp2_to_fp(&E12->E1, &E12_fp2.E1);
    curve_mg_fp2_to_fp(&E12->E2, &E12_fp2.E2);
    curve_mg_fp2_to_fp(&E34->E1, &E34_fp2.E1);
    curve_mg_fp2_to_fp(&E34->E2, &E34_fp2.E2);
