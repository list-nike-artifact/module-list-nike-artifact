...
void
theta_precomputation_fp2(theta_structure_fp2_t *A)
{
    ...
    theta_point_fp2_t A_dual;
    to_squared_theta_fp2(&A_dual, &A->null_point);
...
void
double_point_fp2(theta_point_fp2_t *out, theta_structure_fp2_t *A, const theta_point_fp2_t *in)
{
    to_squared_theta_fp2(out, in);
    fp2_sqr(&out->x, &out->x);
    fp2_sqr(&out->y, &out->y);
    fp2_sqr(&out->z, &out->z);
    fp2_sqr(&out->t, &out->t);
...
void
double_iter_fp2(theta_point_fp2_t *out, theta_structure_fp2_t *A, const theta_point_fp2_t *in, int exp)
{
    if (exp == 0) {
        *out = *in;
    } else {
        double_point_fp2(out, A, in);
        for (int i = 1; i < exp; i++) {
            double_point_fp2(out, A, out);
        }
    }
}

uint32_t
is_product_theta_point_fp2_t(const theta_point_fp2_t *P)
{
    fp2_t t1, t2;
    fp2_mul(&t1, &P->x, &P->t);
    fp2_mul(&t2, &P->y, &P->z);
